...
static real dx (real x, real y, real s) {
    return s * (y - x);
}

static real dy (real x, real y, real z, real r) {
    return r * x - y - x * z;
}

static real dz (real x, real y, real z, real b) {
    return x * y - b * z;
}

int main (int argc, char **argv) {
    long nsteps, interval;
    real x0, y0, z0, sigma, rho, beta, h, _;
    ...
    for (long step = 1; step < nsteps + 1; step++) {
        ...
        if (step % interval == 0) { t_xyz_output(x0, y0, z0, h * step); }
    }
    return 0;
}

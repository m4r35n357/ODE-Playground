...
class Dual:
    ...
    @staticmethod
    def solve(model, x0, εf=1e-15, εx=1e-15, limit=101, debug=False):  # Dual-based Newton solver for Python consoles
        ...
        while abs(f.val) > εf or abs(δx) > εx:
            ...
            x += δx
            if debug:
                print(f"x {x.val:+.3e}  f {f.val:+.3e}  δx {δx:+.3e}  count {count - 1}")
            count += 1
            if count == limit + 1:
                break

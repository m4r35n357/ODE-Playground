...
real sigma (parameters *_) {
    return _->q_r * _->q_r + _->a * _->a * (1.0L - _->sth2.val);
}

pair gamma_v (parameters *_, real sigma) {
    real g = _->v_t / sigma;
    return (pair){g, sqrtl(1.0L - 1.0L / (g * g))};
}

static void refresh (parameters *_) {
    dual r = d_var(_->q_r);
    dual r2 = d_sqr(r);
...
void update_q (parameters *_, real c) {  // dq/dt = d"H"/dp
    _->q_t  += c * _->v_t;
    _->q_r  += c * _->v_r;
    _->q_th += c * _->v_th;
    _->q_ph += c * _->v_ph;
...
void update_p (parameters *_, real d) {  // dp/dt = - d"H"/dq = - (- 0.5 dX/dq) where X is R or THETA

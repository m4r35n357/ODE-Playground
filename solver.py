...
x1 = to_mpfr(argv[3])
tol = to_mpfr(argv[4])
max_it = to_mpfr(argv[5])
target = to_mpfr(argv[6])

model = playground
if (method == Analysis.NA) or (method == Analysis.BI):
    print(bisect(model, x0, x1, x_tol=tol, f_tol=tol, max_it=max, target=target))
if (method == Analysis.NA) or (method == Analysis.FP):
    print(secant(model, x0, x1, x_tol=tol, f_tol=tol, max_it=max, target=target, fp=True))
...
if (method == Analysis.NA) or (method == Analysis.H3):
    print(householder(model, x0, 4, x_tol=tol, f_tol=tol, max_it=max, target=target))

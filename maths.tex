...
\usepackage{listings}
\usepackage{graphicx}
\title{\textbf{Taylor Series Method for Solving ODEs}}
\author{Ian Smith}
\date{December 2018}
\graphicspath{ {images/} }
\begin{document}
...
\abstract
Here is a simple and accurate numerical approach for evolving a system of nonlinear ordinary differential equations (ODEs).
Time varying quantities are represented using arbitrary degree Taylor Series, with the derivative terms in the series generated via recurrence relationships.
Horner's method is then used to calculate the value at the next timestep. 

\section{Introduction}
Starting with a differential equation (ODE), or system of such equations:
\begin{equation}
\dot{x} = f(x)
\end{equation}
We can represent the variable(s) $x$ as a Taylor Series in the time domain.
...
The interval between those initial condition(s) and that later time is the time step, $h$.
...
x_k = \frac{x^k}{k!}
\end{equation}
so that we now have the simpler form:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
but we can also say, as in (4):
\begin{equation}
\dot{x}(h) = \sum_{k=0}^{\infty} \dot{x}_kh^k
\end{equation}
equating the two polynomials by powers of $h$ we get for the $k^{th}$ term:
\begin{equation}
\dot{x}_k = (k + 1) x_{k+1}
\end{equation}
or
...
\end{equation}
This is a key realtionship between the "derivative" of the ODE, and the next term in the taylor series.
\section{Procedure}

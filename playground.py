...
def secant(model, xa, xb, f_tol=to_mpfr(1.0e-12), x_tol=to_mpfr(1.0e-12), max_it=1001, sense=Sense.FLAT, target=zero(+1), mode=Solver.ROOT):
    a, b, c = Series.get(3, xa), Series.get(3, xb), Series.get(3)
...
def analyze(model, method, x0, x1, steps, f_tol, x_tol, max_it, order):
    ...
    if method != Analysis.SKIP.value:
        ...
        elif method == Analysis.SECANT.value:
            print("Secant method", file=stderr)
            ...
            if k > 0:
                ...
                if f1_prev * w_f.jet[1] <= zero(+1):
                    ...
                    if method == Analysis.BISECTION.value:
                        ...
                        yield secant(model, w_x.val, w_x.val + step, f_tol, x_tol, max_it, sense, mode=Solver.EXTREMUM)
                    elif method == Analysis.NEWTON.value:

        ...
        if n != 0:
            if k > 0:
                ...
                if f_prev * w_f.jet[0] < 0.0:
                    ...
                    if n == 1:
                        # noinspection PyUnboundLocalVariable
                        _print_output(bisect(fun, w_x.jet[0], x_prev, f_tol, x_tol, target=target))
                    elif n == 2:
                        _print_output(newton(fun, w_x.jet[0], f_tol, x_tol, target=target))
                    else:
                        _print_output(householder(fun, w_x.jet[0], n, f_tol, x_tol, target=target))
                # noinspection PyUnboundLocalVariable
                if f_dash_prev * w_f.jet[1] < 0.0:
                    ...
                    if n == 1:
                        # noinspection PyUnboundLocalVariable
                        _print_output(bisect(fun, w_x.jet[0], x_prev, f_tol, x_tol, mode=Solver.EXTREMUM))
                    elif n == 2:
                        _print_output(newton(fun, w_x.jet[0], f_tol, x_tol, mode=Solver.EXTREMUM))
                    else:
                        _print_output(householder(fun, w_x.jet[0], n, f_tol, x_tol, mode=Solver.EXTREMUM))
                # noinspection PyUnboundLocalVariable
                if f_dash_dash_prev * w_f.jet[2] < 0.0:
                    ...
                    if n == 1:
                        # noinspection PyUnboundLocalVariable
                        _print_output(bisect(fun, w_x.jet[0], x_prev, f_tol, x_tol, mode=Solver.INFLECTION))
                    elif n == 2:
                        _print_output(newton(fun, w_x.jet[0], f_tol, x_tol, mode=Solver.INFLECTION))
                    else:
                        _print_output(householder(fun, w_x.jet[0], n, f_tol, x_tol, mode=Solver.INFLECTION))
        x_prev = w_x.jet[0]
        f_prev = w_f.jet[0]

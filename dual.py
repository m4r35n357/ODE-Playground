...
def newton_3(j, λ0, εf=1e-12, εx=1e-12, limit=101, sense=Sense.FLAT, debug=False):
...
def analyze_3(j, λa, λb, steps, εf, εx, limit, console=True):
    f0_prev = None
    step = (λb - λa) / (steps - 1)
    for k in range(steps):
        ...
        ce = characteristic_equation(j, λ)
        if not console:
            print(f'{λ.val:.{Context.places}e} {ce}')
        if k > 0:
            if f0_prev * ce.val < 0.0:
                sense = Sense.DECREASING if f0_prev > ce.val else Sense.INCREASING
                yield newton_3(j, λ.val, εf=εf, εx=εx, limit=limit, sense=sense)
        f0_prev = ce.val

def mplot_3(model_a, model_b, model_c, x, y, z, λ_min=-28.0, λ_max=8.0, steps=1000, ce_min=-10.0, ce_max=10.0):
    j = jacobian(model_a, model_b, model_c, x, y, z)
    for result in analyze_3(j, λ_min, λ_max, 1001, 1e-12, 1e-12, 101):
